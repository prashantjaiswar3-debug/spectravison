...
export default function Home() {
  ...
  const handlePrintSticker = () => {
    ...
    if (printWindow && stickerRef.current) {
        ...
        printWindow.document.write('<style>@media print { body { -webkit-print-color-adjust: exact; color-adjust: exact; } @page { size: 3.5in 2in; margin: 0; } } body { margin: 0; font-family: sans-serif; display: flex; justify-content: center; align-items: center; } .sticker { width: 336px; height: 192px; box-sizing: border-box; border: 1px solid #000; padding: 8px; display: flex; flex-direction: column; background: white; color: black; font-size: 14px; } .header { text-align: center; border-bottom: 1px solid #000; padding-bottom: 4px; margin-bottom: 8px; } .title { font-weight: bold; font-size: 1.2rem; } .location { font-size: 0.9rem; } .details-grid { display: grid; grid-template-columns: repeat(4, auto); column-gap: 16px; row-gap: 4px; justify-content: start; } .detail-key { font-weight: bold; } .detail-colon { } .detail-value { grid-column: span 1; } </style>');
